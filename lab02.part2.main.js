...
function getFirstIpAddress(cidrStr, callback) {
  ...
  let mappedAddress = getIpv4MappedIpv6Address(cidrStr);
  ...
  arrayOfValues= {firstIpAddress, mappedAddress} ;

  // Call the passed callback function.
  // Node.js convention is to pass error data as the first argument to a callback.
  // The IAP convention is to pass returned data as the first argument and error
  // data as the second argument to the callback function.
  return callback(arrayOfValues, callbackError);
}
 ...
 */
async function getIpv4MappedIpv6Address(ipv4) {
...
function main() {
  ...
  for (let i = 0; i < sampleCidrsLen; i++) {
    ...
    getFirstIpAddress(sampleCidrs[i], (data, error) => {
      ...
      console.log(`  Response returned from GET request: ${data.firstIpAddress} Mapped Address: ${data.mappedAddress}`);
    });
  }
